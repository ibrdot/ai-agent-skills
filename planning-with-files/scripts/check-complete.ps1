...
if (-not (Test-Path $PlanFile)) {
    Write-Host ('[planning-with-files] No planning file found at ' + $PlanFile + ' -- no active planning session.')
    exit 0
}
...
if ($COMPLETE -eq $TOTAL -and $TOTAL -gt 0) {
    Write-Host ('[planning-with-files] ALL PHASES COMPLETE (' + $COMPLETE + '/' + $TOTAL + '). If the user has additional work, add new phases to ' + $PlanFile + ' before starting.')
} else {
    $PlanningDir = Split-Path -Parent $PlanFile
    Write-Host ('[planning-with-files] Task in progress (' + $COMPLETE + '/' + $TOTAL + ' phases complete). Update ' + (Join-Path $PlanningDir "progress.md") + ' before stopping.')
    if ($IN_PROGRESS -gt 0) {
