...
PLAN_FILE="${1:-${PLANNING_DIR:-.claude/planning}/task_plan.md}"
PROGRESS_FILE="$(dirname "$PLAN_FILE")/progress.md"

if [ ! -f "$PLAN_FILE" ]; then
  echo "[planning-with-files] No planning file found at $PLAN_FILE — no active planning session."
  exit 0
fi
...
if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
  ...
  echo "[planning-with-files] Task in progress ($COMPLETE/$TOTAL phases complete). Update $PROGRESS_FILE before stopping."
